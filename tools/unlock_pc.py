...
# Global State Flag for 2-Step Interactive Unlock
_AWAITING_UNLOCK_PASSWORD = False
...
def is_awaiting_unlock_password():
    """Check if Buddy is waiting for user's password input."""
    global _AWAITING_UNLOCK_PASSWORD
    return _AWAITING_UNLOCK_PASSWORD
...
def wake_windows_lock_screen():
    ...
    try:
        ...
        _AWAITING_UNLOCK_PASSWORD = True

        return (
            "Boss, I sent the SPACE key sequence to awaken your lock screen!\n\n"
            "The password input box is now open on your PC screen.\n"
            "Just type your exact password (e.g. 'yeahbuddy') right here and hit Enter!"
        )
...
def submit_unlock_password(password_text):
    ...
    clean_pwd = re.sub(r"^(?:password|pin|with\s+password|with\s+pin)\s*", "", clean_pwd, flags=re.IGNORECASE).strip()
    ...
    try:
        ...
        return f"Boss, typed exact password '{clean_pwd}' into your lock screen and sent ENTER! Your PC is logging in."
    except Exception as e:
...
def unlock_windows_pc(prompt_payload):
    ...
    - If payload is empty -> Step 1: Wakes screen with SPACE key and waits for user's next message as password.
